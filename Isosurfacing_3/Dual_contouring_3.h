...
bool get_vertex_position(const Domain_ &domain,
  ...
  point = CGAL::ORIGIN +
          (pos[0] + 0.5 * (pos[7] - pos[0])); // set point to voxel center
  ...
  if constexpr (use_bbox) {
    CGAL::Bbox_3 bbox =
        (CGAL::ORIGIN + pos[0]).bbox() + (CGAL::ORIGIN + pos[7]).bbox();
...
void make_quad_mesh_using_dual_contouring(const Domain_ &domain,
                                          const typename Domain_::FT iso_value,
                                          PointRange &points,
                                          PolygonRange &polygons,
                                          const bool use_bbox = false) {
  typedef typename Domain_::FT FT;
  typedef typename Domain_::Point_3 Point_3;
    ...
    for (int k = 0; k < size_k - 1; k++) {
      for (int j = 0; j < size_j - 1; j++) {
        for (int i = 0; i < size_i - 1; i++) {
          ...
          Point_3 p;
          if (get_vertex_position<Domain_, true>(domain, iso_value, i, j, k,
                                                 p)) {
